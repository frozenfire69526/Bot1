...
bot = telebot.TeleBot('7483857382:AAH6Ul08f2--Pr9-y3o7VYdahDz-yxf5pf8')

# Admin user IDs
admin_id = ["2136214036","7212246299"]
...
def show_all_users(message):
    ...
    if user_id in admin_id:
        ...
        response = "Only @FrozenFireFZ Can Run This Command."
    bot.reply_to(message, response)

@bot.message_handler(commands=['owner'])
def show_owner(message):
    response = "👑 Bot Owner: @FrozenFireFZ"  # Replace with the actual owner username
    bot.reply_to(message, response)
        ...
        if len(command) > 2:  # Check if the command contains the user ID and time duration
            ...
            try:
                ...
                if user_to_add not in allowed_user_ids:
                    ...
                    save_user_access(user_access)
                    response = f"User {user_to_add} approved for {time_value} {time_unit} by @FrozenFireFZ.\n\n\n 🅑🅞🅣 🅛🅘🅝🅚: @Frozenfiree"
                else:
...
def handle_bgmi(message):
    ...
    if user_id in allowed_user_ids:
        ...
        response = ("🚫 Unauthorized Access! 🚫\n\n"
                    ...
                    "👉 Contact an Admin or the Owner @FrozenFireFZ for approval.\n"
                    "🌟 Become a proud supporter and purchase approval.\n"
                    "💬 Chat with an Owner @FrozenFireFZ now and level up your capabilities!\n\n"
...
def welcome_start(message):
    ...
    response = (
        f"🥀Welcome {user_name}!\n\n"
        " For User ID : /id \n\n"
        "👉 Join our official channel - https://t.me/Frozenfiree ✅\n\n"
        "👑 For access: @FrozenFireFZ"
        "👑 OWNER : @FrozenFireFZ"
    )
    bot.reply_to(message, response)
...
def clear_logs_handler(message):
    ...
    if user_id in admin_id:
        ...
        bot.reply_to(message, "Only @FrozenFireFZ can run this command.")
